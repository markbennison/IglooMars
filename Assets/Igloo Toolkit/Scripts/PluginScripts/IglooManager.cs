    ...
    public class IglooManager : Singleton<IglooManager>
    {
        ...
        public void CreateIgloo(GameObject parent = null, GameObject follow = null)
        {
            ...
            if (followObject)
            {
                ...
                Debug.Log("<b>[Igloo]</b> follow transform assigned, Igloo player system disabled");
            }

            // ui setup
            UIManager.SetSettings(settings.UISettings);

            if (canvasUI && textureUI && settings.UISettings.useUI)
            {
                cursorUI = canvasUI.GetComponentInChildren<VirtualInputCursor>().GetComponent<RectTransform>();
                if (!cursorUI) Debug.LogWarning("<b>[Igloo]</b> No Cursor UI Prefab has been added to the Canvas UI as a child. Please stop and add one.");
            ...
            else if (canvasUI && textureUI && !settings.UISettings.useUI)
            {
