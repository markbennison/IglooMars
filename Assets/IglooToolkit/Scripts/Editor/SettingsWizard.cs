        ...
        public Settings LoadSettings(SettingsType type, string path = null) {
            ...
            switch (type) {
                case SettingsType.Project:                   
                    settings = serializer.Load(System.IO.Path.Combine(Application.streamingAssetsPath + "/", "IglooSettings"));
                    break;
            ...
            if (settings != null) {
                ...
                if (showDisplay) {
                    ...
                    if (settings.DisplaySettings.useCubemapToEquirectangular) {
                        ...
                        GUILayout.Space(5f); 
                        settings.DisplaySettings.equirectangularTexuteRes = new Vector2IntItem(EditorGUILayout.Vector2IntField("Equirectangular Output Resolution:", settings.DisplaySettings.equirectangularTexuteRes.Vector2Int));
                ...
                if (showSystem) {
                    ...
                    settings.SystemSettings.targetFPS = EditorGUILayout.IntField("Vsync Mode:", settings.SystemSettings.targetFPS);
                }
                else {
