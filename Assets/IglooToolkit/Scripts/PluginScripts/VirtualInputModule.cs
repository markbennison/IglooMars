...
namespace UnityEngine.EventSystems
{
	...
	{
		public string virtualClickButtonName = "IglooButtonA";
		bool virtualPressedThisFrame	= false;
		bool virtualReleasedThisFrame	= false;

		private void Update() {
			if (Input.GetButtonDown(virtualClickButtonName)) virtualPressedThisFrame = true;
			if (Input.GetButtonUp(virtualClickButtonName)) virtualReleasedThisFrame = true;
		}
		...
		[SerializeField]
		private RectTransform m_VirtualCursor = null;
		[SerializeField]
		private Camera m_canvasCamera = null;
		...
		protected void ProcessMousePress(MouseButtonEventData data)
		{
			...
			//if (data.PressedThisFrame() || Input.GetButtonDown(virtualClickButtonName))
			if (data.PressedThisFrame() || virtualPressedThisFrame) {
			...
			//if (data.ReleasedThisFrame() || Input.GetButtonUp(virtualClickButtonName))
			if (data.ReleasedThisFrame() || virtualReleasedThisFrame)
			{
			...
			virtualReleasedThisFrame = false;
			virtualPressedThisFrame = false;
		}
